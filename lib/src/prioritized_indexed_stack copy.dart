...
class PrioritizedIndexedStack extends StatelessWidget {
  ...
  const PrioritizedIndexedStack({
    ...
    this.indexedWrapperBuilder, // New parameter
  });
  ...
  /// Optional callback to wrap active children.
  ///
  /// This builder is called for each child widget that is currently active (i.e.,
  /// its original index is present in the `indices` list). It allows for
  /// applying transformations, opacity, animations, or any other widget wrapping
  /// based on the child's original index (`childOriginalIndex`) in the `children` list,
  /// its stacking order (`stackingOrder` where 0 is topmost), and the child widget itself.
  ///
  /// If a child's index appears multiple times in `indices`, this builder is called
  /// once for that child, using the `stackingOrder` corresponding to its highest
  /// position (i.e., first occurrence) in the `indices` list. The returned wrapped
  /// widget will be used for all occurrences of this child in the stack.
  ///
  /// If this callback is null, children are displayed as is.
  final IndexedWidgetWrapperBuilder? indexedWrapperBuilder;
  ...
  Widget build(BuildContext context) {
    ...
    final wrappedChildren = List<Widget>.generate(children.length, (int i) {
      ...
      if (indexedWrapperBuilder != null && isActive) {
        // stackingOrder is guaranteed non-null here due to isActive check.
        childWidget = indexedWrapperBuilder!(context, i, stackingOrder, childWidget);
      }
